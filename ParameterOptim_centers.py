...
# 实例化函数
func_list = [NonlinearFunctions.Func_N1()]
func = func_list[0]
...
N_count = -1
# 依次搜索N_centers
for N_centers in N_centers_choices:
    print('===================== 中心',N_centers,' ======================')
    N_count = N_count + 1
    
    w = int(0.4 * N_centers)  # 需要进行数据生成的比例
    Euler_distances = []
    Time_expenditure = []
...
df.to_excel('N_centers_results.xlsx',index=False)

df = pd.DataFrame(time_matix)
df.to_excel('N_centers_time.xlsx',index=False)